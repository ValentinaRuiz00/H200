...
class PDFWithAlpha extends FPDF {
    ...
    function SetAlpha($alpha, $blend_mode = 'Normal') {
        if (!in_array($blend_mode, array('Normal', 'Multiply', 'Screen', 'Overlay', 'Darken', 'Lighten', 'ColorDodge', 'ColorBurn', 'HardLight', 'SoftLight', 'Difference', 'Exclusion'))) {
            $blend_mode = 'Normal';
        }
        $this->alpha = array('alpha' => $alpha, 'blend_mode' => $blend_mode);
        $this->_out(sprintf('/%s gs', $this->_getBlendMode()));
    }

    function _getBlendMode() {
        return $this->alpha['blend_mode'] . ($this->alpha['alpha'] < 1 ? 'ca' : '');
    }
}
?>
